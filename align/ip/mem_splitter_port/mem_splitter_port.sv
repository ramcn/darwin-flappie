...
logic [(MULTI_PORT_MODE ? S_WIDTH_A-1 : S_WIDTH_A-1-$clog2(NUM_BANKS)):0] b_arb_address [NUM_BANKS]; // Word addr
...
reg [S_WIDTH_A-1 + MULTI_PORT_MODE:0] slave_address_transformed; // Word addr
...
begin 
  ...
  always@*
  begin
    if (MULTI_PORT_MODE==1)
      slave_address_transformed = {slave_address[BURSTCOUNT_WIDTH-1], slave_address};
    else if (mode_r)
...
mem_router_reorder #(
  ...
  .ADDRESS_W(S_WIDTH_A + MULTI_PORT_MODE), // Warning: icm uses word address!
  .BYTEENA_W(BYTEENABLE_WIDTH),
  .NUM_BANKS(NUM_BANKS),
  .BANK_SEL_BIT(S_WIDTH_A + MULTI_PORT_MODE - 1 - ($clog2(NUM_BANKS) - 1)), // MSBs choose bank
